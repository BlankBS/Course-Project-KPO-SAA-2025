...
void FST::LexAnalyzer(In::IN in, Out::OUT out, Log::LOG log, LT::LexTable& lextable, IT::IdTable& idtable)
{
	...
	libs[1].parcount = 1;
	libs[1].retval = IT::UBYTE;

	libs[2].name = "chr";
	libs[2].params[0] = IT::UBYTE;
	libs[2].parcount = 1;
...
#pragma region "FSTS"
	...
	FST l_verb(
		str,
		2,
		NODE(10, RELATION('+', 1), RELATION('-', 1), RELATION('*', 1),
	...
	Checker checkArr[] = {
		Checker(&l_ubyte,LEX_UBYTE,IT::UBYTE),
		Checker(&l_string,LEX_STRING,IT::CHR),
		Checker(&l_bool,LEX_BOOL ,IT::BOOL),
	...
	for (int i = 0; i < sizze; i++)
	{
		...
		int len = strlen((const char*)(lex.lexem));
		for (int k = 0; k < len; k++)
		{
			str[k] = lex.lexem[k];
		}
		...
		for (int j = 0; j < checkArrLen; j++)
		{
			if (execute(*checkArr[j].chain))
			{
				...
				case LEX_DO: {
					// do-while loop, boolean expression will be checked later
					break;
				}
				case LEX_IF: {
					isBoleanExpression = true;
				...
				case LEX_TYPE: {
					if (checkArr[j].iddatatype == IT::UBYTE)
					{
						dataType = IT::UBYTE;
					}
					...
					else if (checkArr[j].iddatatype == IT::BOOL)
					{
						dataType = IT::BOOL;
					}
					else
				...
				case LEX_LEFTHESIS: {
					...
					{
						if (lextable.table[lextable.size - 4].lexema == LEX_ADD || lextable.table[lextable.size - 4].lexema == LEX_DECLARE)
						{
				...
				case LEX_LITERAL: {
					...
					if (checkArr[j].iddatatype == IT::BOOL)
					{
						short out;
						if (str[0] == 't')
						{
							...
							out = false;
						int positionInTable = IT::IsLX(idtable, IT::BOOL, out);
						if (positionInTable != TI_NULLIDX)
						{
							continueFlag = false;
							lexTableObject.idxTI = positionInTable;
				...
				if (lexTableObject.lexema == LEX_BOOL_OPERATOR)
					lexTableObject.data = str[0];
				if (lexTableObject.lexema == LEX_INVERSION)
					lexTableObject.data = str[0];
