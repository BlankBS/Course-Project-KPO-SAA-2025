					...
					else
					{
						if (shallWrite)
						{
							MFST_TRACE4(log, "TNS_NORULECHAIN/NS_NORULE")
								MFST_TRACE4_CONSOLE("TNS_NORULECHAIN/NS_NORULE")
						}
			...
			else if ((st.top() == lenta[lenta_position]))
			{
				...
				if (shallWrite)
				{
					MFST_TRACE4(log, "TS_NOK/NS_NORULECHAIN")
						MFST_TRACE4_CONSOLE("TS_NOK/NS_NORULECHAIN")
				}
	...
	bool Mfst::savestate(const Log::LOG& log) {
		...
		if (shallWrite)
		{
			MFST_TRACE6(log, "SAVESTATE:", storestate.size())
				MFST_TRACE6_CONSOLE("SAVESTATE:", storestate.size())
		}
		return true;
	};
		...
		case NS_NORULECHAIN:
			if (shallWrite)
			{
				MFST_TRACE4(log, "------>NS_NORULECHAIN")
					MFST_TRACE4_CONSOLE("------>NS_NORULECHAIN")
			}
			break;
		case NS_ERROR:
			if (shallWrite)
	...
	void Mfst::printrules(const Log::LOG& log)
	{
		...
		for (unsigned short k = 0; k < storestate.size(); k++)
		{
			...
			if (shallWrite)MFST_TRACE7(log)
				if (shallWrite)MFST_TRACE7_CONSOLE
		};
	};
