...
namespace CG {
	void InvokeExpressions(std::ofstream* stream, LEX::LEX t, int start, int end, int salt = 0)
	{
		for (int i = start; i < end; i++)
		{
			if (t.lextable.table[i].lexema == LEX_EQUAL && t.lextable.table[i].data == '=')
			{
				...
				*stream << '\n';
				...
				while (true)
				{
					...
					else if (t.lextable.table[i + pos].lexema == LEX_OPERATOR || t.lextable.table[i + pos].lexema == LEX_INVERSION) {
						if (t.lextable.table[i + pos].data != '~')
							*stream << "pop ebx" << '\n';
						*stream << "pop eax" << '\n';
				...
				if (t.idtable.table[t.lextable.table[i + 1].idxTI].iddatatype != IT::STR)
				{
			...
			else if (t.lextable.table[i].lexema == LEX_OUT) {
				switch (t.idtable.table[t.lextable.table[i + 1].idxTI].iddatatype)
				{
				case (IT::CHR): {
					*stream << "push eax\n";
				...
				while (leftSquareCount > 0) {
					if (t.lextable.table[i + pos].lexema == LEX_LEFT_SQUAREBRACE)
						leftSquareCount++;
					else if (t.lextable.table[i + pos].lexema == LEX_RIGHT_SQUAREBRACE)
						leftSquareCount--;
					pos++;
				}
				...
				while (t.lextable.table[conditionStart + conditionPos].lexema != LEX_RIGHTHESIS) {
					...
					case LEX_ID: {
						if (firstId == nullptr)
							firstId = &t.idtable.table[t.lextable.table[conditionStart + conditionPos].idxTI];
						else
							secondId = &t.idtable.table[t.lextable.table[conditionStart + conditionPos].idxTI];
						break;
					}
					case LEX_LITERAL: {
						if (firstId == nullptr)
					...
					case LEX_BOOL_OPERATOR: {
						switch (t.lextable.table[conditionStart + conditionPos].data) {
						case '^': oper = "jne"; break;
						case '!': oper = "je"; break;
						case '<': oper = "jl"; break;
						case '>': oper = "ja"; break;
				...
				*stream << "\nDoWhile" << doWhileNumber << "Start: " << '\n';
				...
				if (firstId != nullptr) {
					...
					*stream << "cmp eax, ebx" << '\n';
					*stream << oper << " DoWhile" << doWhileNumber << "Start" << '\n';
				}
					...
					case LEX_LITERAL: {
						if (getExpressionParams)
						{
							if (firstId == nullptr)
								...
								secondId = &t.idtable.table[t.lextable.table[i + pos].idxTI];
							if (t.lextable.table[i + pos + 1].lexema != LEX_BOOL_OPERATOR)
								soloBool = true;
	...
	void Head(std::ofstream* stream, LEX::LEX t) {
		...
		*stream << "\tincludelib ../Debug/SAA-2025LIB.lib\n";
		...
		for (int i = 0; i < t.idtable.size; i++)
		{
			if (t.idtable.table[i].idtype == IT::L)
			{
				*stream << "\t" << t.idtable.table[i].id;
				...
				case IT::UBYTE: {
					*stream << " BYTE " << t.idtable.table[i].value.vubyte << " ;UBYTE";
					break;
				}
				case IT::CHR: {
				...
				case IT::BOOL: {
					*stream << " BYTE " << t.idtable.table[i].value.vbool << " ;BOOL";
					break;
				}
				case IT::STR: {
