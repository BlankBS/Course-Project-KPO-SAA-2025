...
namespace Parm
{
	Parm::PARM getparm(int argc, _TCHAR* argv[])
	{
		...
		{
			if (argv[i][0] == 't' && argv[i][1] == 'r' && argv[i][2] == 'e' && argv[i][3] == 'e' && argv[i][4] == '\0') {
				out.showMfst = true;
			}
			if (argv[i][0] == 'l' && argv[i][1] == 'e' && argv[i][2] == 'x' && argv[i][3] == '\0') {
				out.showTables = true;
			}
			if (argv[i][0] == 'i' && argv[i][1] == 'd' && argv[i][2] == '\0') {
		...
		else
		{
			if (wcslen(out.out) == 0)
			{
				...
				wcscpy_s(out.out, a);
				//wcscpy_s(out.out, L"..\\SAA-2025\\SAA-2025ASM.asm");
			}
			if (wcslen(out.log) == 0)
			{
				wchar_t a[PARM_MAX_SIZE];
